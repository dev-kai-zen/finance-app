...
export function saveAccountType(input: AccountTypeInput, id?: string): string {
  ...
  return db.transaction((tx) => {
    ...
    if (existing && existing.accountGroup !== value.accountGroup) throw new Error("An account type's group cannot be changed.");
    if (existing && isProtectedAccountType(existing) && existing.name !== value.name) throw new Error("System account type names cannot be changed.");
    requireUniqueTypeName(value.name, value.accountGroup, id, tx);
    const now = new Date();
    if (existing) {
      updateAccountTypeRecord(existing.id, { name: value.name, iconKey: value.iconKey, color: value.color, updatedAt: now }, tx);
      return existing.id;
    }
    const newId = newAccountRecordId(tx);
    const sortOrder = Math.max(-1, ...listAccountTypes(tx).filter((t) => t.accountGroup === value.accountGroup).map((t) => t.sortOrder)) + 1;
    insertAccountType({ ...value, id: newId, sortOrder, createdAt: now, updatedAt: now }, tx);
    return newId;
  });
}
